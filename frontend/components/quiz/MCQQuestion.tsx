...
type MCQQuestionProps = {
  question: Pick<QuizQuestionResponse, "id" | "prompt" | "options" | "hint">;
  value: string;
    ...
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="secondary">{question.options.length || 0} options</Badge>
        </div>
      ...
      <CardContent className="space-y-4">
        {question.options.length > 0 ? (
          ...
          <div className="empty-state py-8">
            <p className="text-sm text-muted-foreground">No answer options were returned for this question.</p>
          </div>
        )}
